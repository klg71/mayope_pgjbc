...
public enum PGProperty {
  ...
  ADAPTIVE_FETCH(
      "adaptiveFetch",
      "false",
      "Specifies if number of rows fetched in ResultSet should be adaptive to maxResultBuffer and" +
          " max row size."),
  ...
  /**
   * Specifies what the driver should do if a query fails. In {@code autosave=always} mode, JDBC
   * driver sets a savepoint before each query,
   * and rolls back to that savepoint in case of failure. In {@code autosave=never} mode
   * (default), no savepoint dance is made ever.
   * In {@code autosave=conservative} mode, savepoint is set for each query, however the rollback
   * is done only for rare cases
   * like 'cached statement cannot change return type' or 'statement XXX is not valid' so JDBC
   * driver rollsback and retries
   */
  AUTOSAVE(
      "autosave",
      "never",
      "Specifies what the driver should do if a query fails. In autosave=always mode, JDBC driver" +
          " sets a savepoint before each query, "
          + "and rolls back to that savepoint in case of failure. In autosave=never mode " +
          "(default), no savepoint dance is made ever. "
          + "In autosave=conservative mode, safepoint is set for each query, however the rollback" +
          " is done only for rare cases"
          + " like 'cached statement cannot change return type' or 'statement XXX is not valid' " +
          "so JDBC driver rollsback and retries",
      false,
   ...
   * This schema will be used to resolve
  ...
  /**
   * Specifies the maximum number of fields to be cached per connection. A value of {@code 0}
   * disables the cache.
   */
  ...
  /**
   * Specifies the maximum size (in megabytes) of fields to be cached per connection. A value of
   * {@code 0} disables the cache.
   */
  DATABASE_METADATA_CACHE_FIELDS_MIB(
      "databaseMetadataCacheFieldsMiB",
      "5",
      "Specifies the maximum size (in megabytes) of fields to be cached per connection. A value " +
          "of {@code 0} disables the cache."),
  ...
  DEFAULT_ROW_FETCH_SIZE(
      "defaultRowFetchSize",
      "0",
      "Positive number of rows that should be fetched from the database when more rows are needed" +
          " for ResultSet by each fetch iteration"),
  ...
  /**
   * Specifies how the driver transforms JDBC escape call syntax into underlying SQL, for
   * invoking procedures or functions. (backend &gt;= 11)
   * In {@code escapeSyntaxCallMode=select} mode (the default), the driver always uses a SELECT
   * statement (allowing function invocation only).
   * In {@code escapeSyntaxCallMode=callIfNoReturn} mode, the driver uses a CALL statement
   * (allowing procedure invocation) if there is no return parameter specified, otherwise the
   * driver uses a SELECT statement.
   * In {@code escapeSyntaxCallMode=call} mode, the driver always uses a CALL statement (allowing
   * procedure invocation only).
   */
  ESCAPE_SYNTAX_CALL_MODE(
      ...
      "Specifies how the driver transforms JDBC escape call syntax into underlying SQL, for " +
          "invoking procedures or functions. (backend >= 11)"
          + "In escapeSyntaxCallMode=select mode (the default), the driver always uses a SELECT " +
          "statement (allowing function invocation only)."
          + "In escapeSyntaxCallMode=callIfNoReturn mode, the driver uses a CALL statement " +
          "(allowing procedure invocation) if there is no return parameter specified, otherwise " +
          "the driver uses a SELECT statement."
          + "In escapeSyntaxCallMode=call mode, the driver always uses a CALL statement (allowing" +
          " procedure invocation only).",
      false,
  ...
  /**
   * Enable mode to filter out the names of database objects for which the current user has no
   * privileges
  ...
  HIDE_UNPRIVILEGED_OBJECTS(
      ...
      "false",
      "Enable hiding of database objects for which the current user has no privileges granted " +
          "from the DatabaseMetaData"),

  HOST_RECHECK_SECONDS(
      "hostRecheckSeconds",
      "10",
      "Specifies period (seconds) after which the host status is checked again in case it has " +
          "changed"),
  ...
  LOAD_BALANCE_HOSTS(
      "loadBalanceHosts",
      "false",
      "If disabled hosts are connected in the given order. If enabled hosts are chosen randomly " +
          "from the set of suitable candidates"),
  ...
  LOG_SERVER_ERROR_DETAIL(
      ...
      "true",
      "Include full server error detail in exception messages. If disabled then only the error " +
  ...
  LOG_UNCLOSED_CONNECTIONS(
      ...
      "false",
      "When connections that are not explicitly closed are garbage collected, log the stacktrace " +
          "from the opening of the connection to trace the leak source"),
   ...
   * The value of this parameter may contain spaces and other special characters or their URL
   * representation.
   */
  OPTIONS(
      "options",
  ...
  /**
   * <p>Specifies which mode is used to execute queries to database: simple means ('Q' execute,
   * no parse, no bind, text mode only),
   * extended means always use bind/execute messages, extendedForPrepared means extended for
   * prepared statements only,
   * extendedCacheEverything means use extended protocol and try cache every statement (including
   * Statement.execute(String sql)) in a query cache.</p>
   *
   * <p>This mode is meant for debugging purposes and/or for cases when extended protocol cannot
   * be used (e.g. logical replication protocol)</p>
   */
  PREFER_QUERY_MODE(
      "preferQueryMode",
      "extended",
      "Specifies which mode is used to execute queries to database: simple means ('Q' execute, no" +
          " parse, no bind, text mode only), "
          + "extended means always use bind/execute messages, extendedForPrepared means extended " +
          "for prepared statements only, "
          + "extendedCacheEverything means use extended protocol and try cache every statement " +
          "(including Statement.execute(String sql)) in a query cache.", false,
  ...
  PREPARED_STATEMENT_CACHE_QUERIES(
      ...
      "256",
      "Specifies the maximum number of entries in per-connection cache of prepared statements. A " +
          "value of {@code 0} disables the cache."),
  ...
  PREPARED_STATEMENT_CACHE_SIZE_MIB(
      ...
      "5",
      "Specifies the maximum size (in megabytes) of a per-connection prepared statement cache. A " +
          "value of {@code 0} disables the cache."),
  ...
  PROTOCOL_VERSION(
      "protocolVersion",
      null,
      "Force use of a particular protocol version when connecting, currently only version 3 is " +
          "supported.",
      false,
      new String[]{"3"}),
  ...
  READ_ONLY_MODE(
      "readOnlyMode",
      "transaction",
      "Controls the behavior when a connection is set to be read only, one of 'ignore', " +
          "'transaction', or 'always' "
          + "When 'ignore', setting readOnly has no effect. "
          + "When 'transaction' setting readOnly to 'true' will cause transactions to BEGIN READ " +
          "ONLY if autocommit is 'false'. "
          + "When 'always' setting readOnly to 'true' will set the session to READ ONLY if " +
          "autoCommit is 'true' "
  ...
  /**
   * <p>Connection parameter passed in the startup message. This parameter accepts two values;
   * "true"
  ...
  /**
   * Parameter controlling where ssl encryption ends. The allowed values are {@code server},
   * {@code proxy}
   * Default options is "server"
   */
  SSL_TERMINATION(
      "ssltermination",
      "server",
      "Parameter governing the termination of SSL",
      false,
      new String[]{"proxy", "server"}),
  ...
  SSL_PASSWORD_CALLBACK(
      "sslpasswordcallback",
      null,
      "A class, implementing javax.security.auth.callback.CallbackHandler that can handle " +
          "PassworCallback for the ssl password."),
  ...
  STRING_TYPE(
      "stringtype",
      null,
      "The type to bind String parameters as (usually 'varchar', 'unspecified' allows implicit " +
  ...
  TARGET_SERVER_TYPE(
      ...
      false,
      new String[]{"any", "primary", "master", "slave", "secondary", "preferSlave",
          "preferSecondary"}),
   ...
   * Legacy behavior with external entity processing can be enabled by specifying a value of
   * LEGACY_INSECURE.
